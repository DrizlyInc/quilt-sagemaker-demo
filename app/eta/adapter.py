...
class Adapter:
    def __init__(self, redis_host="redis", model_filename="/opt/ml/model/composed_model.pkl"):
    ...
    def predict(self, request):
        ...
        for eta_request_dict in request["eta_requests"]:
            ...
            try:
                eta_request_arr = self._data_service.get_data(eta_request_dict, hod=hod, dow=dow)
            ...
            #That the data service pulls are defined by another application that can (but shouldn't) have gaps
                ...
                #TODO: Right now errors in the data service are indiscriminantly thrown up here
                # this might be better handled by custom exceptions within that class for the cleanliness of logging
                tb = sys.exc_info()[-1]
                stk = traceback.extract_tb(tb)
                fname = stk[-1][-1]
                model_prediction = self._data_service.get_eta_fallback(eta_request_dict)
                logging.info(f"Fallback model for:{eta_request_dict} returned an error in '{fname}' fallback_predictions:{model_prediction}")
    ...
    #TODO: Rewrite for batch processing
    # The ensemble model might be too slow
...
if __name__ == '__main__':
    #requires redis to be running local
    import cProfile
    adapter = Adapter(redis_host="localhost", model_filename="data/composed_model.pkl")
    eta_request = {"eta_requests": [{"store_id": -1, "delivery_zipcode": 11550, "hod":17, "dow":'Fri'}]}
